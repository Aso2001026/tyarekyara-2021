...
if(!empty($_POST)){
    ...
    if(!isset($error)){
        ...
        $id = $_SESSION['user']['id'];
        $url = "http://keen-akune-3587.bambina.jp/ANYPORT/changeMailCompletion/changeMailCompletion.php?urltoken=".$urltoken."&id=".$id;
        //ここでデータベースに登録する
        try{
            //例外処理を投げる（スロー）ようにする
            $sql = "INSERT INTO m_pre_users (pre_user_token,user_id, pre_user_mail, reg_date, flag) VALUES (:pre_user_token,:user_id, :pre_user_mail, now(), '2')";
            $stm = $pdo->prepare($sql);
